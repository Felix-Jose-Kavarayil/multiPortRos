...
parser.add_argument("-l", "--lightDuration",help="set the light duration in seconds",type=int, action="store",default = "1")
...
parser.add_argument("-p", "--probeTrialProportion",help="set the proportion of probe trials (from 0 to 1)",type=float, action="store",default = ".10")
...
lightDurationSec = args.lightDuration
...
print("sessionName:",sessionName,"Session duration:",sessionDurationSec,"Light duration:",lightDurationSec,"Refractory on reward:",refractoryDurationSec, "Probe trial proportion:",probeTrialProportion)
print("file:",fileBase)

rospy.init_node('multiport_task')


pubCommand = rospy.Publisher('multi_port_control',Int32,queue_size=1)
...
nextLightChange = lastLightChange+lightDurationSec
    ...
    if time.time() > nextLightChange:
        if lightStatus == 0: # light will turn on
            ...
            if myRand < probeTrialProportion:
                isProbeTrial=True
                msg.frame_id="probe"
                msg.stamp=rospy.get_rostime()+ rospy.Duration(lightDurationSec/2)  # add time so it is in the middle of the light interval
                pubTaskEvent.publish(msg)
            else:
                isProbeTrial=False
            lightStatus=1
            ...
            pubCommand.publish(0)
            lightStatus=0
        nextLightChange = time.time()+lightDurationSec
